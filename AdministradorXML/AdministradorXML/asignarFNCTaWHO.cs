...
namespace AdministradorXML
{
    public partial class asignarFNCTaWHO : Form
    {
        ...
        private void asignarFNCTaWHO_Load(object sender, EventArgs e)
        {
            ...
            String queryDP = "SELECT ANL_CODE FROM [" + Properties.Settings.Default.sunDatabase + "].[dbo].[" + Login.unidadDeNegocioGlobal + "_ANL_CODE] WHERE ANL_CAT_ID= '04' order by ANL_CODE asc";
            try
            {
                using (SqlConnection connection = new SqlConnection(connString))
                {
